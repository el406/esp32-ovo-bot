...
static i2s_dev_t *I2S[I2S_NUM_1] = {&I2S0, &I2S1};
...
esp_err_t i2s_parallel_driver_install(i2s_port_t port,
                                      ...
                                      bool invert_clk, intr_handler_t irq_hndlr,
                                      void *priv) {
  ...
  // Third stage config, width of data to be written to IO (I think this should
  // always be the actual data width?)
  ...
  // No support for fractional dividers (could probably be ported from official
  // serial i2s driver though)
  dev->clkm_conf.val = 0;
  ...
  // Not really described for non-pcm modes, although datasheet states it should
  // be set correctly even for LCD mode First stage config. Configures how data
  // is loaded into fifo
  if (conf->sample_width == I2S_PARALLEL_WIDTH_24) {
    ...
    // once. Length of fifo must thus still be word-alligned
  ...
  if (irq_hndlr) {
    esp_err_t err =
        esp_intr_alloc(irq_source, ESP_INTR_FLAG_IRAM, irq_hndlr, priv, NULL);
    if (err) {
      return err;
    }
